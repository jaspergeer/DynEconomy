...
package com.jaspergeer.dyneconomy.commands;
...
public class EconomyCommands implements CommandExecutor {
    ...
    public EconomyCommands(DynEconomy plugin) {
        ...
        currency = Material.matchMaterial(
                Objects.requireNonNull(plugin.getConfig().getString("currency_item")));
        Objects.requireNonNull(plugin.getCommand("dyneconomy")).setExecutor(this);
        plugin.getLogger().info("Enabled Commands");
    }
    ...
    /**
     * Player requests a quantity of money be take from another player or
     * themselves and the specified Player loses the quantity of money.
     * Sub-command of /dyneconomy.
     *
     * @param p Player to take money from
     * @param args index 1 contains the amount of money
     */
    void takeCommand(Player p, String[] args) {
        if (Array.getLength(args) > 2) {
            Player s = Bukkit.getPlayerExact(args[1]);
            if (s == null) {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (Array.getLength(args) > 0) {
            if (args[0].equalsIgnoreCase("sell")) {
                ...
                if (p.hasPermission("dyneco.player")) {
                    infoCommand(p);
                } else {
                    p.sendMessage(Utils.chat("&cYou don't have permission to do that!"));
                }
